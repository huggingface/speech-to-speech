...
import sounddevice as sd    # cspell: disable-line
...
def listen_and_play(
    ...
    try:
        ...
        print(f"Failed to connect to send port: {e}")
        return
    ...
    try:
        ...
        print(f"Failed to connect to recv port: {e}")
        return

    print("Recording and streaming...")

    # Create threading event and queues
    ...
    def callback_recv(outdata, frames, time, status): 
        if status:
            print(f"Status error in recv callback: {status}")
    ...
    def callback_send(indata, frames, time, status):
        if status:
            print(f"Status error in send callback: {status}")
        if recv_queue.empty():
            data = bytes(indata)
            send_queue.put(data)
