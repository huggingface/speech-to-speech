...
def test_demo_clients_track_overlapping_user_transcriptions_by_item(module_path, class_name, message_handler):
    node = shutil.which("node")
    if node is None:
        pytest.skip("Node.js is required for demo client tests")
...
def test_demo_clients_clear_partial_after_empty_final_transcript(module_path, class_name, message_handler):
    node = shutil.which("node")
    if node is None:
        pytest.skip("Node.js is required for demo client tests")
...
def test_demo_clients_clear_partial_after_transcription_failure(module_path, class_name, message_handler):
    node = shutil.which("node")
    if node is None:
        pytest.skip("Node.js is required for demo client tests")
...
await deliver({{
  ...
  item_id: "item_1",
  content_index: 0,
  error: {{ type: "transcription_error", code: "backend_error", message: "failed" }},
}});
...
if (client._userTranscriptByItem.has("item_1")) {{
  throw new Error("failed transcription did not prune the partial transcript");
}}
"""
    subprocess.run(
        [node, "--input-type=module", "-e", script],
