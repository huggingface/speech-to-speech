...
class TestDispatchPipelineEvent:
    ...
    def test_new_turn_merges_with_unanswered_previous_user_message(self, runtime_config, should_listen):
        text_prompt_queue = Queue()
        tracker = SpeculativeTurnTracker()
        service = RealtimeService(
            text_prompt_queue=text_prompt_queue,
        ...
        service.dispatch_pipeline_event(
            conn_id,
            TranscriptionCompletedEvent(transcript="show me a dance move", turn_id="turn_1", turn_revision=0),
        )
        first_item_id = service._state(conn_id).speculative_user_item_id

        service.dispatch_pipeline_event(
            conn_id,
            TranscriptionCompletedEvent(transcript="poor guy", turn_id="turn_2", turn_revision=0),
        )

        user_items = [item for item in runtime_config.chat.buffer if getattr(item, "role", None) == "user"]
        assert len(user_items) == 1
        assert user_items[0].id == first_item_id
        assert user_items[0].content[0].text == "show me a dance move\npoor guy"
        first_req = text_prompt_queue.get_nowait()
        second_req = text_prompt_queue.get_nowait()
        assert first_req.turn_id == "turn_1"
        assert second_req.turn_id == "turn_2"
        assert service._state(conn_id).speculative_user_item_id == first_item_id
        assert service._state(conn_id).speculative_user_item_closed is False
        service.unregister(conn_id)

    def test_new_turn_does_not_merge_after_assistant_output(self, runtime_config, should_listen):
        text_prompt_queue = Queue()
        tracker = SpeculativeTurnTracker()
        service = RealtimeService(
            text_prompt_queue=text_prompt_queue,
        ...
        service.dispatch_pipeline_event(
            conn_id,
            AssistantTextEvent(text="sure", turn_id="turn_1", turn_revision=0),
        )
        service.finish_audio_response(conn_id)
        ...
        assert [item.content[0].text for item in user_items] == ["show me a dance move", "poor guy"]
        assert service._state(conn_id).speculative_user_item_closed is False
        service.unregister(conn_id)

    def test_same_turn_revision_does_not_replace_after_assistant_output(self, runtime_config, should_listen):
        text_prompt_queue = Queue()
        tracker = SpeculativeTurnTracker()
        service = RealtimeService(
            text_prompt_queue=text_prompt_queue,
        ...
        service.dispatch_pipeline_event(
            conn_id,
            TranscriptionCompletedEvent(transcript="actually, a jump", turn_id="turn_1", turn_revision=0),
        )

        user_items = [item for item in runtime_config.chat.buffer if getattr(item, "role", None) == "user"]
        assert len(user_items) == 2
        assert [item.content[0].text for item in user_items] == ["show me a dance move", "actually, a jump"]
        assert service._state(conn_id).speculative_user_item_closed is False
        service.unregister(conn_id)
