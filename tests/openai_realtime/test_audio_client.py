...
def test_audio_client_prints_terminal_transcript_for_legacy_done_only_server(capsys):
    playback = PlaybackBuffer(16000)
    renderer = _FriendlyEventRenderer()
...
def test_audio_client_does_not_repeat_streamed_transcript_after_tool_event(capsys):
    playback = PlaybackBuffer(16000)
    renderer = _FriendlyEventRenderer()
...
def test_audio_client_keeps_json_events_off_live_assistant_line(capsys):
    playback = PlaybackBuffer(16000)
    renderer = _FriendlyEventRenderer()

    class JsonEvent(SimpleNamespace):
        def model_dump_json(self):
            return f'{{"type":"{self.type}"}}'

    for event in (
        JsonEvent(type="response.output_audio_transcript.delta", delta="Hello"),
        JsonEvent(type="response.output_audio_transcript.delta", delta=" there."),
        JsonEvent(type="response.output_audio_transcript.done", transcript="Hello there."),
    ):
...
def test_audio_client_does_not_repeat_partial_transcript_after_response_error(capsys):
    playback = PlaybackBuffer(16000)
    renderer = _FriendlyEventRenderer()
    ...
    for event in (
        ...
        SimpleNamespace(
            type="error",
            error=SimpleNamespace(type="response_failed", message="generation failed"),
        ),
        SimpleNamespace(type="response.output_audio_transcript.done", transcript="partial"),
        SimpleNamespace(type="response.done", response=SimpleNamespace(status="failed")),
    ):
        handle_server_event(event, playback=playback, renderer=renderer, print_json=False)

    assert capsys.readouterr().out == (
        "ASSISTANT: partial\nERROR: response_failed: generation failed\nASSISTANT: <response failed>\n"
    )


async def test_audio_client_finishes_live_assistant_line_on_disconnect(monkeypatch, capsys):
    class FakeStream:
        def start(self):
            pass
    ...
    class DisconnectingConnection:
        ...
        async def recv(self):
            if not self.received:
                self.received = True
                return SimpleNamespace(type="response.output_audio_transcript.delta", delta="partial")
