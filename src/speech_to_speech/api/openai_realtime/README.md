...
Internal partial transcriptions are cumulative hypotheses. The Realtime server emits only the newly appended suffix as `conversation.item.input_audio_transcription.delta`. If a later hypothesis revises text that was already emitted, that partial is withheld because the protocol has no transcript-retraction event. Clients should treat `conversation.item.input_audio_transcription.completed` as authoritative and replace any rendered partial for the same `item_id` with its final `transcript`. Turn metadata routes out-of-order completions to their originating item; when a metadata-less completion arrives while multiple items are unresolved, it receives a standalone item rather than being assigned to the current turn. Server transcript-prefix buffers are capped at 128 unresolved items. After a prefix is evicted, further partials for that item are withheld because the emitted prefix is no longer available; item routing and duration are retained so the authoritative completion and its usage remain accurate. Bundled clients retain unresolved item transcripts until completion so later deltas and empty authoritative completions remain correct.
